# Arduino Component Glossary

A quick reference guide for all the hardware components used in the Master Hub.


## 1. Resistor
Limits the flow of electrical current to protect delicate components.
[Explore Resistor in detail](https://en.wikipedia.org/wiki/Resistor)


## 2. Capacitor
Stores and releases electrical energy, acting like a tiny battery.
[Explore Capacitor in detail](https://en.wikipedia.org/wiki/Capacitor)


## 3. Diode
Allows electricity to flow in only one direction, acting like a one-way valve.
[Explore Diode in detail](https://en.wikipedia.org/wiki/Diode)


## 4. Light Emitting Diode (LED)
A special type of diode that emits light when current passes through it.
[Explore LED in detail](https://en.wikipedia.org/wiki/Light-emitting_diode)


## 5. Transistor
Acts as a digital switch or amplifier for electrical signals.
[Explore Transistor in detail](https://en.wikipedia.org/wiki/Transistor)


## 6. Breadboard
A plug-and-play board for building temporary circuits without soldering.
[Explore Breadboard in detail](https://en.wikipedia.org/wiki/Breadboard)


## 7. Jumper Wires
Used to connect different components together on a breadboard.
[Explore Jumper Wires in detail](https://en.wikipedia.org/wiki/Jump_wire)


## 8. Arduino Uno
The microcontroller board that acts as the brain for your projects.
[Explore Arduino Uno in detail](https://en.wikipedia.org/wiki/Arduino)


## 9. Microcontroller
A compact integrated circuit designed to govern a specific operation.
[Explore Microcontroller in detail](https://en.wikipedia.org/wiki/Microcontroller)


## 10. Integrated Circuit (IC)
A tiny chip containing thousands of microscopic transistors.
[Explore Integrated Circuit in detail](https://en.wikipedia.org/wiki/Integrated_circuit)


## 11. Potentiometer
A variable resistor that you can turn like a volume knob.
[Explore Potentiometer in detail](https://en.wikipedia.org/wiki/Potentiometer)


## 12. Photoresistor (LDR)
A sensor that changes its resistance based on the amount of light.
[Explore Photoresistor in detail](https://en.wikipedia.org/wiki/Photoresistor)


## 13. Thermistor
A temperature sensor whose resistance changes with heat.
[Explore Thermistor in detail](https://en.wikipedia.org/wiki/Thermistor)


## 14. Piezo Buzzer
A small speaker that generates sound when voltage is applied.
[Explore Piezo Buzzer in detail](https://en.wikipedia.org/wiki/Piezoelectric_speaker)


## 15. Push Button
A simple mechanical switch that closes a circuit when pressed.
[Explore Push Button in detail](https://en.wikipedia.org/wiki/Push-button)


## 16. Relay
An electromechanical switch used to control high-voltage devices.
[Explore Relay in detail](https://en.wikipedia.org/wiki/Relay)


## 17. Servo Motor
A motor that can rotate to precise angles, usually between 0 and 180 degrees.
[Explore Servo Motor in detail](https://en.wikipedia.org/wiki/Servomotor)


## 18. Stepper Motor
A motor that rotates in discrete, precise steps.
[Explore Stepper Motor in detail](https://en.wikipedia.org/wiki/Stepper_motor)
**Used in:** [Project 12: Stepper Motor Basics](../projects/12_stepper_motor/), [Project 36: Stepper Motor](../projects/36_stepper_motor/)


## 19. DC Motor
A standard electric motor that spins continuously when powered.
[Explore DC Motor in detail](https://en.wikipedia.org/wiki/DC_motor)


## 20. Ultrasonic Sensor (HC-SR04)
Measures distance by bouncing sound waves off objects.
[Explore Ultrasonic Sensor in detail](https://en.wikipedia.org/wiki/Ultrasonic_sensor)


## 21. PIR Motion Sensor
Detects movement by sensing infrared heat emitted by bodies.
[Explore PIR Motion Sensor in detail](https://en.wikipedia.org/wiki/Passive_infrared_sensor)


## 22. Humidity Sensor (DHT11)
Measures the amount of water vapor in the air.
[Explore Humidity Sensor in detail](https://en.wikipedia.org/wiki/Hygrometer)


## 23. Tilt Sensor
Detects orientation or inclination using a tiny metal ball inside.
[Explore Tilt Sensor in detail](https://en.wikipedia.org/wiki/Tilt_sensor)


## 24. Flame Sensor
Detects the specific infrared light wavelengths emitted by fire.
[Explore Flame Sensor in detail](https://en.wikipedia.org/wiki/Flame_detector)


## 25. Real-Time Clock (RTC)
Keeps accurate track of the date and time using a coin-cell battery, even if the main Arduino loses power.
[Explore Real-Time Clock in detail](https://en.wikipedia.org/wiki/Real-time_clock)


## 26. LCD Display (16x2)
A screen capable of displaying 2 rows of 16 text characters.
[Explore LCD Display in detail](https://en.wikipedia.org/wiki/Liquid-crystal_display)


## 27. 7-Segment Display
An array of 7 LEDs arranged to display numbers 0-9.
[Explore 7-Segment Display in detail](https://en.wikipedia.org/wiki/Seven-segment_display)


## 28. Membrane Keypad
A flat array of buttons used for number and code entry.
[Explore Membrane Keypad in detail](https://en.wikipedia.org/wiki/Membrane_keyboard)


## 29. RFID Reader
Reads wireless smart cards and key fobs using radio frequencies.
[Explore RFID Reader in detail](https://en.wikipedia.org/wiki/Radio-frequency_identification)
**Used in:** [Project 24: RFID Door Lock](../projects/24_rfid_reader/), [Project 37: RFID Reader](../projects/37_rfid_reader/)


## 30. IR Receiver
Decodes infrared flashes from devices like a TV remote.
[Explore IR Receiver in detail](https://en.wikipedia.org/wiki/Infrared_Data_Association)


## 31. Shift Register
Expands the number of digital output pins on your Arduino.
[Explore Shift Register in detail](https://en.wikipedia.org/wiki/Shift_register)


## 32. Voltage Regulator
Maintains a constant output voltage regardless of input changes.
[Explore Voltage Regulator in detail](https://en.wikipedia.org/wiki/Voltage_regulator)


## 33. ESP8266 Module
Adds WiFi capabilities to your Arduino projects.
[Explore ESP8266 Module in detail](https://en.wikipedia.org/wiki/ESP8266)


## 34. Bluetooth Module (HC-05)
Allows wireless serial communication with phones and computers.
[Explore Bluetooth Module in detail](https://en.wikipedia.org/wiki/Bluetooth)
**Used in:** [Project 38: Bluetooth HC-05](../projects/38_bluetooth_hc05/)


## 35. Soil Moisture Sensor
Measures the conductivity of dirt to detect if plants need water.
[Explore Soil Moisture Sensor in detail](https://en.wikipedia.org/wiki/Soil_moisture_sensor)


## 36. Water Level Sensor
Detects depth by measuring resistance across parallel conductive traces.
[Explore Water Level Sensor in detail](https://en.wikipedia.org/wiki/Level_sensor)
**Used in:** [Project 22: Water Level](../projects/22_water_level/), [Project 40: Water Level Sensor](../projects/40_water_level_sensor/)


## 37. Laser Diode
Emits a highly focused, concentrated beam of light.
[Explore Laser Diode in detail](https://en.wikipedia.org/wiki/Laser_diode)


## 38. Logic Level Shifter
Safely converts signals between devices operating at different voltages (typically 5V and 3.3V).
[Explore Logic Level Shifter in detail](https://en.wikipedia.org/wiki/Level_shifter)


## 39. Motor Driver Module (e.g., L298N or L293D)
Acts as a high-current amplifier that allows low-power Arduino pins to control the speed and direction of high-power DC motors or stepper motors.
[Explore Motor Driver Module in detail](https://en.wikipedia.org/wiki/Motor_controller)
**Used in:** [Project 35: L298N Motor Driver](../projects/35_l298n_motor_driver/)


## 40. Gas Sensor (MQ Series, e.g., MQ-2)
Detects the presence and concentration of various gases in the air, such as LPG, smoke, propane, methane, and carbon monoxide.
[Explore Gas Sensor in detail](https://en.wikipedia.org/wiki/Gas_detector)
**Used in:** [Project 33: MQ-2 Gas Sensor](../projects/33_gas_sensor/)


## 41. OLED Display (e.g., SSD1306)
A crisp, power-efficient miniature screen used to display complex graphics, custom fonts, and data readouts without requiring a backlight.
[Explore OLED Display in detail](https://en.wikipedia.org/wiki/OLED)
**Used in:** [Project 39: OLED Display](../projects/39_oled_display/)


## 42. Sound Sensor Module (e.g., KY-038)
Detects ambient noise levels or specific sound spikes. This is a great choice if you're building a voice-controlled obstacle-avoiding robot.
[Explore Sound Sensor Module in detail](https://en.wikipedia.org/wiki/Microphone)


## 43. Joystick Module (e.g., KY-023)
Provides two-axis analog input (X and Y) plus a central digital push-button. This is a highly interactive and heavily requested input device for beginners to play with.
[Explore Joystick Module in detail](https://en.wikipedia.org/wiki/Joystick)
**Used in:** [Project 16: Joystick Servo](../projects/16_joystick_servo/), [Project 34: Joystick Module](../projects/34_joystick_module/)


## 44. RGB LED
Contains red, green, and blue light-emitting diodes in a single package. By adjusting the voltage sent to each pin, you can mix them to emit almost any color of light.
[Explore RGB LED in detail](https://en.wikipedia.org/wiki/Light-emitting_diode)


## 45. Micro SD Card Module
Allows the Arduino to read and write data to a standard microSD card using SPI communication. Essential for data logging, storing large arrays, or keeping project data saved offline.
[Explore Micro SD Card Module in detail](https://en.wikipedia.org/wiki/SD_card)


## 46. Accelerometer and Gyroscope Module (e.g., MPU6050)
Measures acceleration and rotational velocity, perfect for keeping drones stable, building self-balancing robots, or adding motion tracking to your projects. 
[Explore Accelerometer in detail](https://en.wikipedia.org/wiki/Accelerometer)


## 47. Rotary Encoder
An electro-mechanical input device that provides infinite rotation while measuring the direction and amount of turn, often featuring a built-in push button for menu navigation.
[Explore Rotary Encoder in detail](https://en.wikipedia.org/wiki/Rotary_encoder)


## 48. Barometric Pressure Sensor (e.g., BMP280)
Measures atmospheric pressure and temperature, allowing you to calculate precise altitude deviations for weather stations or flight systems.
[Explore Barometric Pressure Sensor in detail](https://en.wikipedia.org/wiki/Barometer)


## 49. Active Piezo Buzzer
Differs from a passive buzzer by generating a fixed-frequency audible tone automatically as soon as direct current power is applied.
[Explore Active Piezo Buzzer in detail](https://en.wikipedia.org/wiki/Buzzer)


## 50. Infrared (IR) Transmitter
Emits modulated infrared light waves to control external appliances like TVs and air conditioners wirelessly.
[Explore IR Transmitter in detail](https://en.wikipedia.org/wiki/Infrared_blaster)
**Used in:** [Project 32: IR Transmitter](../projects/32_ir_transmitter/)


## 51. Optocoupler (Optoisolator)
Transfers electrical signals between two isolated circuits using a short optical path, protecting sensitive microcontrollers from high voltages.
[Explore Optocoupler in detail](https://en.wikipedia.org/wiki/Opto-isolator)


## 52. PIR Motion Sensor
Detects movement by sensing infrared heat emitted by bodies.
**Used in:** [Project 41: PIR Motion Sensor](../projects/41_pir_motion/)


## 53. Sound Sensor Module
Detects ambient noise levels or specific sound spikes.
**Used in:** [Project 42: Sound Sensor Module](../projects/42_sound_sensor/)


## 54. Rotary Encoder
Provides infinite rotation while measuring the direction and amount of turn.
**Used in:** [Project 43: Rotary Encoder](../projects/43_rotary_encoder/)


## 55. 4x4 Keypad
A matrix keypad used for number and code entry.
**Used in:** [Project 44: 4x4 Keypad](../projects/44_4x4_keypad/)


## 56. Relay Module
An electrically operated switch used to control high-power circuits.
**Used in:** [Project 45: Relay Module](../projects/45_relay_module/)


## 57. RTC DS3231
Highly accurate real-time clock.
**Used in:** [Project 46: RTC DS3231](../projects/46_rtc_ds3231/)


## 58. Micro SD Card Module
Allows reading and writing to an SD card.
**Used in:** [Project 47: SD Card Module](../projects/47_sd_card_module/)


## 59. Soil Moisture Sensor
Measures the volumetric water content in soil.
**Used in:** [Project 48: Advanced Soil Moisture](../projects/48_soil_moisture_advanced/)


## 60. Flame Sensor
Detects fire or other wavelength of light.
**Used in:** [Project 49: Flame Sensor](../projects/49_flame_sensor/)


## 61. Pulse Sensor
Used to measure heart rate.
**Used in:** [Project 50: Heart Rate Pulse Sensor](../projects/50_pulse_sensor/)
